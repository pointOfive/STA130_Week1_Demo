...
4. [ ] Discuss loadable packages like `tidyverse` and `tibble` which add functions  
   like `tidyverse::read_csv()` and `tibble::read_csv()` to the R calculator
6. [ ] Introduce the `<cmd-shift-M>` (Mac) and `<ctrl-shift-M>` (PC) "pipe symbol" ` %>%  
6. [ ] Introduce `<-`, `->`, and `=` variable assignments in R **[source/console pane]**
7. [ ] Show variable and data set management and the `rm()` function **[environment pane]**
